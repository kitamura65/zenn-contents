...
もう1つ、惜しいものもありました。Gmailのアドオン向けに`gmail.addons.current.action.compose`というスコープがあって、公式の説明は "Allows the add-on to temporarily create new drafts messages and replies."(アドオンが一時的に下書きや返信を作成できるようにする)。**送信には触れていません。**

つまりGoogleは「下書きの作成だけ」という粒度を、表現できるところでは表現しています。ただしこれはアドオンの実行中だけ使える別枠のスコープで、今回のようなスタンドアロンのスクリプトからは使えませんし、`users.drafts.create`の「Authorization scopes」にも載っていません。普通のアプリが要求できるスコープの側に、それが用意されていないというだけでした。
...
- なぜそうなっているのかは、公式の説明が見つからなかった。「メールボックスに書けるのは危険だから分離できない」という擁護を考えてみたが、`gmail.insert`(メールボックスに追加できるが送信はできない)という反例が同じ表の中にあって成り立たなかった。アドオン向けには下書き作成だけを許すスコープもあるが、アドオンの実行中しか使えない
